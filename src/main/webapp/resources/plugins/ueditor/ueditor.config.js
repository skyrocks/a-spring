...
(function () {
    ...
    /*var URL =  window.UEDITOR_HOME_URL || '/eas/ueditor/';*/
	
    var path = URL.substr(0,URL.length-8 );
    /*var localPath = 'http://localhost:8080/eas/';*/
    ...
    window.UEDITOR_CONFIG = {
        ...
        ,savePath: [ 'ueditor' ]    //图片保存在服务器端的目录， 默认为空， 此时在上传图片时会向服务器请求保存图片的目录列表，
                                                            // 如果用户不希望发送请求， 则可以在这里设置与服务器端能够对应上的目录名称列表
                                                            //比如： savePath: [ 'upload1', 'upload2' ]

        //涂鸦图片配置区
        ,scrawlUrl:URL+"jsp/scrawlUp.jsp"           //涂鸦上传地址
        ,scrawlPath:path                  //图片修正地址，同imagePath

        //附件上传配置区
        ,fileUrl:URL+"/jsp/fileUp.jsp"               //附件上传提交地址
        ,filePath:path                 //附件修正地址，同imagePath
        //,fileFieldName:"upfile"                    //附件提交的表单名，若此处修改，需要在后台对应文件修改对应参数

        //远程抓取配置区
        //,catchRemoteImageEnable:true               //是否开启远程图片抓取,默认开启
        ,catcherUrl:path+"ueditor/jsp/getRemoteImage.jsp"   //处理远程图片抓取的地址
        ,catcherPath:path                 //图片修正地址，同imagePath
        //,catchFieldName:"upfile"                   //提交到后台远程图片uri合集，若此处修改，需要在后台对应文件修改对应参数
        //,separater:'ue_separate_ue'               //提交至后台的远程图片地址字符串分隔符
        //,localDomain:[]                            //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地,默认不抓取127.0.0.1和localhost

        //图片在线管理配置区
        ,imageManagerUrl:path+"ueditor/jsp/imageManager.jsp"       //图片在线管理的处理地址
        ...
        , toolbars:[
            [ 'source', '|', 'undo', 'redo', '|',
					                ...
					                'insertimage', 'attachment', 'map', 'gmap', 'background', '|',
...
//           ]
    ...
    function getUEBasePath ( docUrl, confUrl ) {
    	...
    	var UEBasePath = getBasePath( docUrl || self.document.URL || self.location.href, confUrl || getConfigFilePath() );
